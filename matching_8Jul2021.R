...
#and 0-1 if there was deforestation between years 
#(I know the order seems wrong, but this way we avoid negative numbers)

w_matched_yr$defor.1 <- w_matched_yr$for2005-w_matched_yr$for2006
w_matched_yr$defor.2 <- w_matched_yr$for2006-w_matched_yr$for2007
w_matched_yr$defor.3 <- w_matched_yr$for2007-w_matched_yr$for2008
w_matched_yr$defor.4 <- w_matched_yr$for2008-w_matched_yr$for2009
w_matched_yr$defor.5 <- w_matched_yr$for2009-w_matched_yr$for2010
w_matched_yr$defor.6 <- w_matched_yr$for2010-w_matched_yr$for2011
w_matched_yr$defor.7 <- w_matched_yr$for2011-w_matched_yr$for2012
w_matched_yr$defor.8 <- w_matched_yr$for2012-w_matched_yr$for2013
w_matched_yr$defor.9 <- w_matched_yr$for2013-w_matched_yr$for2014
...
w_matched_yr_subs <- w_matched_yr %>%
  dplyr::select(CFM, PA, cfm_id, pa_id, dist_cart, dist_road, dist_urb, dist_vil, DVSP, edge_05, edge_10, edge_14, elev, rice, precip, slope, veg_type,  pop.1:pop.9, defor.1:defor.9)
...
w_matched_yr_reorg <- w_matched_yr_bind %>% pivot_longer(
  ...
  names_to = "time", #this creates a column with values pop.1 pop.2 etc. not 1, 2 as desired
  names_prefix = "t", #not sure what this does
  values_to = "pop"
)
...
#write to CSV
write_csv(w_matched_yr_reorg,'outputs/w_matched_yr_reorg_8Jul2021b.csv') #update date
