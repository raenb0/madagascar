...
cfm_pa_data_90m_no_na <- drop_na(cfm_pa_data_90m) #remove sample points with NA values #NOTE lose around 10,000 observations - distance from forest edge has MANY NA values, maybe don't exclude NA values??
...
covs <- cfm_pa_data_90m[cov.names]
...
matched <- rbind(cfm_pa_data_90m[m1$index.treated,],cfm_pa_data_90m[m1$index.control,])
