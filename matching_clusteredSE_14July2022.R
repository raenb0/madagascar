...
cfm_90m_data <- read_csv("data/sample_points/cfm05_90m_data.csv") #note all columns read in as double
...
#check that all three data frames have same number of variables (162)

#check to make sure datasets have the same columns:

janitor::compare_df_cols(cfm_90m_data, pa_90m_data)
janitor::compare_df_cols(cfm_90m_data, cfm_rnw_90m_data)
janitor::compare_df_cols(cfm_rnw_90m_data, pa_90m_data)
...
cfm_90m_data <- rename(cfm_90m_data,
                       ...
                       dist_urb = disturb,
                       dist_vil = distvil,
                       #fordens2005 = fordens200, #unecessary if cleaned up CSV file first
                       #fordens2010 = fordens201,
                       #fordens2014 = fordens2_1,
                       pa_id = paid,
                       veg_type = vegtype) 
...
names(cfm_rnw_90m_data) #repeat for Renewed CFM data

cfm_rnw_90m_data <- rename(cfm_rnw_90m_data,
                       cfm_id = cfmid,
                       dist_cart = distcart,
                       dist_road = distroad,
                       dist_urb = disturb,
                       dist_vil = distvil,
                       #density2005 = fordens200,
                       #density2010 = fordens201,
                       #density2014 = fordens2_1,
                       pa_id = paid,
                       veg_type = vegtype) 

names(cfm_rnw_90m_data) #check if renaming worked
...
write_csv(cfm_pa_data_90m,'outputs/cfm_pa_data_90m_with_na_13July2022.csv') #version with NA values

#drop GEE distance from forest edge columns (too many NA values)
cfm_pa_data_90m_no_dist <- dplyr::select(cfm_pa_data_90m, -4:-19)

#remove sample points with NA values, otherwise matching won't work
cfm_pa_data_90m_no_na <- drop_na(cfm_pa_data_90m_no_dist)

#write to CSV
write_csv(cfm_pa_data_90m_no_na,'outputs/cfm_pa_data_90m_no_na_13July2022.csv') #update date, this includes all CFM sample points


# # Convert rice prices to Madagascar currency - DO THIS AFTER MATCHING?? ------------------------------
# 
# #load data if needed
# cfm_pa_data_90m_no_na <- read_csv('outputs/cfm_pa_data_90m_no_na_13July2022.csv') #update date
...
#                        precip_av = precip)
# 
# ##Might need to add a UID here to distinguish unique sample points later?
# 
# #pivot longer
# cfm_pa_data_longer <- cfm_pa_data_subs %>%
#   pivot_longer(
...
# 
# #View
# View(cfm_pa_data_wider) #WORKED!
# 
# cfm_pa_data_wider <- cfm_pa_data_wider %>% mutate(year = as.double(year))
# 
# #load exchange rate data
# MDG_exchange <- read_csv("data/MDG_exchange_rates.csv")
# 
# cfm_pa_data_exch <- left_join(cfm_pa_data_wider, MDG_exchange, by = c("year" = "Year")) %>%
#   mutate(riceav_mdg = riceav*Exchange_rate) %>%
#   mutate(ricesd_mdg = ricesd*Exchange_rate)
# 
# names(cfm_pa_data_exch)
# 
# #---------------------------------------------------
...
cov.names <- c("dist_cart","dist_road","dist_urb","dist_vil","DVSP","edge05","elev","pop2005","precip","rice","slope","veg_type") # Names of covariates used to match **NOTE alphabetical order, included population 2005 and distance to forest edge 2005 from Vieilledent
...
endTime <- Sys.time()
print(endTime - startTime) #1 minute
...
startTime <- Sys.time() #to record time required to run this
mb1 <- MatchBalance(Treat ~ dist_cart + dist_road + dist_urb + dist_vil + DVSP + edge05 + elev + pop2005 + precip + rice + slope + veg_type,
                    match.out = m1, nboots = 500, data=cfm_pa_data_90m_no_na)
endTime <- Sys.time()
print(endTime - startTime) #only takes ~40 seconds
...
write_csv(matched, 'outputs/mahalanobis_matched_13July2022.csv') #update date
...
mb2 <- lm(CFM ~ dist_cart + dist_road + dist_urb + dist_vil + DVSP + edge05 + elev + pop2005 + precip + rice + slope + veg_type, data = cfm_pa_data_90m_no_na)

summary(mb2) #F-statistic: 841.9 on 12 and 46813 DF,  p-value: < 2.2e-16

#AFTER MATCHING: 

mb3 <- lm(CFM ~ dist_cart + dist_road + dist_urb + dist_vil + DVSP + edge05 + elev + pop2005 + precip + rice + slope + veg_type, data = matched)

summary(mb3) #F-statistic: 18.96 on 12 and 23061 DF,  p-value: < 2.2e-16
...
# 
# ## CHECK COVARIATE BALANCE
# mb2 <- MatchBalance(Treat ~ dist_cart + dist_road + dist_urb + dist_vil + DVSP + edge05 + elev + pop2005 + precip + rice + slope + veg_type,
...
matched <- read_csv('outputs/mahalanobis_matched_13July2022.csv')  #update dates
names(matched)
...
# so POSITIVE values indicate DECREASE in forest cover (INCREASE in deforestation)

matched_2pr <- matched %>% 
  mutate(defor.1 = for2005-for2010) %>%
  mutate(defor.2 = for2010-for2014)
...
matched_2pr <- matched_2pr %>% 
  mutate(dens.1 = fordens2005 - fordens2010) %>% #will return a 0 if there was no change and >0 if there was change
  mutate(dens.2 = fordens2010 - fordens2014)

#add distance to forest edge (Vieilledent data) at start of each period

matched_2pr <- matched_2pr %>% 
  mutate(edge.1 = edge05) %>%
  mutate(edge.2 = edge10)

#add control variables (mean during each time period)

matched_2pr <- matched_2pr %>% 
  rowwise() %>% mutate(pop.1 = mean(c(pop2005:pop2009))) %>%
  rowwise() %>% mutate(pop.2 = mean(c(pop2010:pop2014))) %>%
  rowwise() %>% mutate(riceav.1 = mean(c(riceav2005:riceav2009))) %>%
  rowwise() %>% mutate(riceav.2 = mean(c(riceav2010:riceav2014))) %>%
  rowwise() %>% mutate(ricesd.1 = mean(c(ricesd2005:ricesd2009))) %>%
  rowwise() %>% mutate(ricesd.2 = mean(c(ricesd2010:ricesd2014))) %>%
...
matched.subs <- matched_2pr %>%
  dplyr::select(CFM, PA, cfm_id, pa_id, dist_cart, dist_road, dist_urb, dist_vil, DVSP, elev, rice, precip, slope, veg_type,  defor.1:wind.2) #grab all new columns at the end
...
#write to CSV
write_csv(matched_wider,'outputs/matched_wider_13July2022.csv') #update date
...
did_m2 <- plm(dens ~ CFM*period + pop + riceav + ricesd + drght + maxprecip + temp + wind,
              data = matched_wider, 
              effect="twoways", 
              model = "within", 
              index = c("UID", "period"))
...
## REORGANIZE MAHALANOBIS MATCHED DATA FOR ANNUAL ANALYSIS -----------------------##updated July 2022
...
#rename "precip" variable so it doesn't cause issues later
w_matched_yr <- rename(w_matched_yr,
                             precip_av = precip)
...
#write to CSV
write_csv(w_matched_yr_wider,'outputs/w_matched_yr_wider_13July2022.csv') #update date!
...
#load data if needed
w_matched_yr_wider <- read_csv('outputs/w_matched_yr_wider_13July2022.csv') #update date!
